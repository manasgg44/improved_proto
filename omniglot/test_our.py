...
from utils import get_accuracy_proto
import gc
...
def test(args):
    ...
    model_enc = PrototypicalNetwork(1,
                                args.embedding_size,
                                hidden_size=args.hidden_size)
                                    
    model_full = PrototypicalNetwork_new(1, args.embedding_size, args.num_ways, hidden_size=args.hidden_size)
                                    
    # Save model
    if args.output_folder is not None:
        filename1 = os.path.join(args.output_folder, 'protonet_enc_omniglot_'
            '{0}shot_{1}way.pt'.format(args.num_shots, args.num_ways))
        #filename2 = os.path.join(args.output_folder, 'protonet_full_omniglot_'
            #'{0}shot_{1}way.pt'.format(args.num_shots, args.num_ways))
    ...
    with tqdm(dataloader, total=args.num_batches) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                optimizer = torch.optim.SGD(model_full.parameters(), momentum = 0.9, lr=inner_lr)
