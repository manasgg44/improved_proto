...
import random
import numpy as np
from torchmeta.datasets.helpers import omniglot, miniimagenet, fc100
    ...
    with tqdm(dataloader, total=args.num_batches) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            with torch.no_grad():
                ...
                accuracy_original, distance_original = get_accuracy_proto(prototypes, test_embeddings, test_targets)
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                optimizer = torch.optim.SGD([{'params': model_full.encoder.parameters(), 'momentum':0.9, 'lr':inner_lr},
                    {'params': model_full.classifier.parameters(), 'momentum':0.9, 'lr':inner_lr}])
            ...
            with torch.no_grad():
                ...
                accuracy, distance = get_accuracy_proto(prototypes, test_embeddings, test_targets)
                pbar.set_postfix(accuracy='{0:.4f}'.format(accuracy.item()))
                total_accuracy += accuracy
                gc.collect()
                
        print(f"\nTotal accuracy ours {total_accuracy/args.num_batches}")
        print(f"\nTotal accuracy vanilla proto {total_accuracy_original/args.num_batches}")
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--inner_lr', type=int, default=1e-04,
    #    help='Inner Learning rate  (default: 1e-04).')
